			...
			for (int i = 0; i < this.chapter.Articles.size() / 6; i++)
			{
				for (int j = 0; j < 4; j++)
				{
					...
					if (art.Icon instanceof Item)
						this.drawTexturedModalRect(k + 20 + 18 * i, l + 20 + 18 * j, 0, 0, (int) ((Item) art.Icon).getIcon(new ItemStack((Item) art.Icon), 0).getMaxU(),
								(int) (int) ((Item) art.Icon).getIcon(new ItemStack((Item) art.Icon), 0).getMaxV());
