		...
		if (!p.isSneaking())
		{
			if (i != null)
			{
				...
				if (i.getItemDamage() == 1)
				{
					if (i.getTagCompound() != null)
					{
						NBTTagCompound tag = StructureApi.createStructureTag(w,
								AxisAlignedBB.getBoundingBox(x, y, z, i.getTagCompound().getInteger("BlockX"), i.getTagCompound().getInteger("BlockY"), i.getTagCompound().getInteger("BlockZ")), false, Blocks.bedrock, BlockRegistry.mark);
