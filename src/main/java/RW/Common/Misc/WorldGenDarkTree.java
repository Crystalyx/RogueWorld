...
/**
 * @author Lord_Crystalyx using vanilla treeGen Code
 */
public class WorldGenDarkTree extends WorldGenAbstractTree
{
	...
	public WorldGenDarkTree(boolean p_i45448_1_)
	{
		super(p_i45448_1_);
	}
	...
	void func_150529_a(int x, int y, int z, float p_150529_4_, byte p_150529_5_, Block b)
	{
		...
		for (aint1[p_150529_5_] = aint[p_150529_5_]; i1 <= l; ++i1)
		{
			...
			while (j1 <= l)
			{
				...
				else
				{
					...
					if (!block1.isAir(worldObj, aint1[0], aint1[1], aint1[2]) && block1 != BlockRegistry.SLeaves)
					{
						++j1;
					}
					else
					{
						this.setBlockAndNotifyAdequately(this.worldObj, aint1[0], aint1[1], aint1[2], b, 0);
						++j1;
					}
				}
			}
	...
	void generateLeafNode(int p_76491_1_, int p_76491_2_, int p_76491_3_)
	{
		...
		for (int i1 = p_76491_2_ + this.leafDistanceLimit; l < i1; ++l)
		{
			...
			this.func_150529_a(p_76491_1_, l, p_76491_3_, f, (byte) 1, BlockRegistry.SLeaves);
		}
	}

	void func_150530_a(int[] p_150530_1_, int[] p_150530_2_, Block b)
	{
		...
		if (aint2[b1] != 0)
		{
			...
			for (int j = aint2[b1] + b4; i != j; i += b4)
			{
				...
				this.setBlockAndNotifyAdequately(this.worldObj, aint3[0], aint3[1], aint3[2], b, b5);
			}
		}
	}
	...
	void generateTrunk()
	{
		...
		this.func_150530_a(aint, aint1, BlockRegistry.SWood);
	...
	void generateLeafNodeBases()
	{
		...
		for (int[] aint = new int[] { this.basePos[0], this.basePos[1], this.basePos[2] }; i < j; ++i)
		{
			...
			{
				this.func_150530_a(aint, aint2, BlockRegistry.SWood);
			}
		}
	}
