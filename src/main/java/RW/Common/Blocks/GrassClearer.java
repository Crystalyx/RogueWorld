	...
	public boolean onBlockActivated(World w, int x, int y, int z, EntityPlayer p, int meta, float px, float py, float pz)
	{
		...
		for(int i= -rad;i<rad+1;i++)
		{
			for(int l= -hei;l<hei+1;l++)
			{
				...
				{
					if(w.getBlock(x+i, y+l, z+j)==Blocks.tallgrass || w.getBlock(x+i, y+l, z+j)==Blocks.double_plant || w.getBlock(x+i, y+l, z+j)==Blocks.yellow_flower|| w.getBlock(x+i, y+l, z+j)==Blocks.red_flower)
					{
						flag=true;
