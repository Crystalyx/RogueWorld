...
public class TileEntityPillarRender extends TileEntitySpecialRenderer
{
	...
	public void renderTileEntityAt(TileEntity tile, double x,double y, double z, float p_147500_8_) 
	{
        ...
        if(tilee.getStackInSlot(0) != null)
        {
        	...
        	MiscUtils.renderItemStack_Full(Monoficate(tilee.getStackInSlot(0)),tilee.xCoord+0.5D,tilee.yCoord+10D, tilee.zCoord+0.5D, x, y+1.1D, z, rotation,0F,(float) col.getRed()/256,(float) col.getGreen()/256,(float) col.getBlue()/256, 0.5F, 0.175F,0.5F);
        }
    	GL11.glPopMatrix();
